...
def on_powers_controllers(game, notification):
    ...
    """
    if (
        Game.is_player_game(game)
        and notification.powers[game.power.name] != game.power.get_controller()
    ):
        ...
        Game.update_powers_controllers(
            game, notification.powers, notification.timestamps, notification.player_types
        )
...
def on_game_processed(game, notification):
    ...
    """
    game.set_phase_data(
        [notification.previous_phase_data, notification.current_phase_data], clear_history=False
    )
...
def on_omniscient_updated(game, notification):
    ...
    new_game.notification_callbacks.update(
        {key: value.copy() for key, value in game.notification_callbacks.items()}
    )
